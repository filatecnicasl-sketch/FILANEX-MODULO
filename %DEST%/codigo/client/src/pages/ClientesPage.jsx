    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={onCerrar}>
      <div className="modal-panel w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={guardar} className="space-y-5">
          ...
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 pt-2">Datos bancarios</p>
          <div>
          ...
          <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 pt-2">Dirección de entrega</p>
          <div>
              ...
              <tbody>
                {lista.map((c) => (
                  <tr key={c._id}>
                        ...
                        <div className="min-w-0">
                          <p className="font-medium text-[#0f172a] truncate">
                            ...
                            {c.esAdministracionPublica && (
                              <span className="ml-2 text-[10px] font-bold px-1.5 py-0.5 rounded bg-violet-100 text-violet-600 align-middle">
                                FACe
                              </span>
                            )}
                          </p>
                          <p className="text-[11px] text-slate-400 truncate">{c.email || "Sin email"}</p>
                        </div>
                      </div>
                    </td>
                    <td className="num text-[12px] text-slate-500 whitespace-nowrap">{c.nif}</td>
                    <td className="num text-[12px] text-slate-500 whitespace-nowrap">
                      {c.telefono ? (
                    ...
                    <td className="text-slate-500 max-w-[200px] truncate">{c.direccion?.ciudad ?? "—"}</td>
