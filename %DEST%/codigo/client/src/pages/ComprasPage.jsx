            ...
            <tbody className="divide-y divide-white/5">
              {(doc.lineas ?? []).map((l, i) => (
                <tr key={i}>
                  ...
                  <td className="py-2 text-right text-slate-400">{euros(l.precioUnitario)}</td>
                  <td className="py-2 text-right text-slate-400">{l.iva}%</td>
                  <td className="py-2 text-right text-slate-200">
                    {euros((l.cantidad ?? 0) * (l.precioUnitario ?? 0))}
                  </td>
                </tr>
              ))}
            </tbody>
...
export default function ComprasPage() {
  ...
  async function subir(e) {
    ...
    try {
      ...
      if (datos.tipo === "albaran") {
        setNoticias((n) => [
          `Albarán de ${datos.documento?.ocr?.datosExtraidos?.proveedor?.nombre ?? "proveedor desconocido"} registrado como pendiente de facturar.`,
          ...n,
        ]);
      }
      await cargar();
