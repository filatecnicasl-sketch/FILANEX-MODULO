...
import { IconImprimir, IconFirma, IconEditar, IconBorrar } from "../components/icons.jsx";
import { imprimirDocumento } from "../utils/imprimir.js";

// Totales de un albarán a partir de sus líneas.
function totalesDe(a) {
  const base = (a.lineas ?? []).reduce((s, l) => s + (Number(l.cantidad) || 0) * (Number(l.precioUnitario) || 0), 0);
  const iva = (a.lineas ?? []).reduce((s, l) => s + (Number(l.cantidad) || 0) * (Number(l.precioUnitario) || 0) * ((Number(l.iva) || 0) / 100), 0);
...
export default function AlbaranesPage() {
  ...
  return (
    <>
      ...
      {albaranes.length === 0 && !mostrarForm ? (
                    ...
                    <button
                      onClick={() =>
                        imprimirDocumento({
                          ...
                          fecha: a.fecha,
                          contraparte: a.cliente,
                          lineas: a.lineas,
                          notas: a.notas,
                          firma: a.firmaEntrega,
                        })
                      }
                      title="Imprimir"
