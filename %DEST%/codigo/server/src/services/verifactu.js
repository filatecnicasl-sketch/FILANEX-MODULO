...
// Requiere AEAT_CERT_PFX y AEAT_CERT_PASS en el entorno.
export function remitirAeat(xml) {
  return new Promise((resolve, reject) => {
    const ruta = process.env.AEAT_CERT_PFX;
    if (!ruta || !fs.existsSync(ruta)) {
      return reject(
        new Error("Certificado AEAT no configurado (AEAT_CERT_PFX en server/.env)")
      );
    }
