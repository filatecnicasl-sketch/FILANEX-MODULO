...
router.post("/:id/emitir", async (req, res, next) => {
  try {
    ...
    // TODO: numeración atómica ($inc) cuando haya concurrencia real.
    const { serie: nombreSerie, numero, serieNumero } = tomarNumero(empresa, "facturaVenta");
    await empresa.save();
    ...
    // Remisión inmediata solo si hay certificado configurado; si no, queda
    // pendiente para reintento (la AEAT admite subsanación de remisiones).
    if (process.env.AEAT_CERT_PFX) {
      try {
        const resp = await remitirAeat(xml);
        ...
        factura.verifactu.enviada = aceptado || conErrores;
        factura.verifactu.estadoEnvio = registro.estadoEnvio;
        await registro.save();
        await factura.save();
      } catch (e) {
...
router.post("/:id/rectificativa", async (req, res, next) => {
  try {
    ...
    const { serie, numero, serieNumero } = tomarNumero(empresa, "facturaVenta");
    await empresa.save();
