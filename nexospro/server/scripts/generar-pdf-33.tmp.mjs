// Genera el PDF de la factura 3-2026-33 (filanex_local) ya con sello PAGADA.
...
await conContexto({ dbName: "filanex_local", slug: "local" }, async () => {
  ...
  const factura = await FacturaVenta.findById(ID).populate("cliente", "nombre nif direccion");
  ...
  fs.writeFileSync("/tmp/factura-3-2026-33-pagada.pdf", pdf);
