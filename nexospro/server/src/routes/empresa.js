...
import { uploadMemoria, guardarSubida, borrarSubida } from "../middleware/upload.js";
...
async function borrarLogosAnteriores(empresa) {
  if (!empresa?.logoUrl) return;
  await borrarSubida(empresa.logoUrl).catch(() => {});
}
...
// Si la instalación aún no tiene empresa (primera configuración del
// asistente), se crea con estos datos.
...
router.post("/logo", [uploadMemoria.single("archivo"), contextoTrasSubida], async (req, res, next) => {
  try {
    const empresa = await Empresa.findOne();
    if (!empresa) return res.status(404).json({ error: "No hay empresa configurada" });
    ...
    await borrarLogosAnteriores(empresa);
    const logoUrl = await guardarSubida(req, "logos", "logo-empresa");
