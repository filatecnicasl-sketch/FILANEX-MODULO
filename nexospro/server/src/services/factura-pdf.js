...
export async function generarPdfFactura({ empresa, factura, cliente }) {
  ...
  // Tabla de líneas. Si alguna línea tiene descuento, se añade la columna
  // Dto% y el importe sale neto (cantidad × precio × (1 - dto/100)).
  const lineas = factura.lineas ?? [];
  const conDto = lineas.some((l) => (l.descuento ?? 0) > 0);
  const top = 200;
  doc.font("Helvetica-Bold").fontSize(9);
  doc.text("Descripción", 50, top);
  if (conDto) {
    doc.text("Uds.", 300, top, { width: 40, align: "right" });
    doc.text("Precio", 345, top, { width: 55, align: "right" });
    doc.text("Dto%", 403, top, { width: 35, align: "right" });
  } else {
  ...
  for (const l of lineas) {
    ...
    if (conDto) {
      ...
      doc.text((l.descuento ?? 0) > 0 ? `${l.descuento}%` : "", 403, y, { width: 35, align: "right" });
    } else {
