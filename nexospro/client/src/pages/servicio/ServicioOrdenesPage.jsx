...
import { ESTADOS_OS, tonoEstado } from "./datos.js";
import RecepcionRapidaServicio from "./RecepcionRapidaServicio.jsx";
import FormOrdenServicio from "./FormOrdenServicio.jsx";
import MenuImprimirOrdenServicio from "../../components/MenuImprimirOrdenServicio.jsx";
import ModalRecepcionServicio from "./ModalRecepcionServicio.jsx";
...
// Servicio a domicilio (el técnico se desplaza): píldora teal.
function BadgeDomicilio({ tipoServicio }) {
  if (tipoServicio !== "domicilio") return null;
  return (
    <span
      title="Servicio a domicilio: el técnico se desplaza"
      className="inline-block rounded-full border bg-teal-100 text-teal-700 border-teal-200 text-[0.625rem] font-semibold px-1.5 py-px ml-1.5 align-middle"
    >
      Domicilio
    </span>
  );
...
export default function ServicioOrdenesPage() {
  ...
  const [recepcionOS, setRecepcionOS] = useState(undefined); // orden en recepción digital
  ...
  async function cargar() {
    try {
      const r = await fetch("/api/servicio/ordenes");
  ...
  async function cambiarEstado(o, estado) {
    const r = await fetch(`/api/servicio/ordenes/${o._id}`, {
      method: "PUT",
  ...
  async function facturar(o) {
    ...
    const r = await fetch(`/api/servicio/ordenes/${o._id}/facturar`, { method: "POST" });
    const datos = await r.json();
    if (r.ok) {
      if (window.confirm("Factura creada en Ventas como borrador. ¿Quieres ir a Ventas para validarla y emitirla?")) {
  ...
  const filtrada = (lista ?? []).filter((o) =>
    coincideBusqueda(q, o.numero, o.aparatoDescripcion, o.clienteNombre, o.aparato?.numeroSerie)
  );

  return (
    <>
      <CabeceraPagina titulo="Órdenes de servicio" descripcion="Reparaciones de informática y electrónica, en tienda y a domicilio.">
      ...
      <div className="mb-4">
        <InputBusqueda
          ...
          placeholder="Buscar por nº, aparato, cliente o nº de serie…"
        />
      </div>
      ...
      <div className="panel px-3.5 py-2">
        {!lista ? null : filtrada.length === 0 ? (
          <EstadoVacio
            titulo={q ? `Sin resultados para «${q}»` : "Sin órdenes de servicio"}
            descripcion="La recepción rápida crea la orden y da de alta el aparato en un paso."
          />
        ) : (
                ...
                {filtrada.map((o) => (
                  <tr key={o._id}>
                    ...
                    <td className="text-slate-300">
                      {o.aparatoDescripcion ?? "—"}
                      {o.aparato?.numeroSerie && (
                        <p className="text-[0.6875rem] text-slate-500 num">S/N {o.aparato.numeroSerie}</p>
                      )}
                    </td>
                    <td className="text-slate-300">{o.clienteNombre ?? "—"}</td>
                    <td className="text-slate-400 max-w-[220px] truncate" title={o.averia ?? ""}>{o.averia || "—"}</td>
                    ...
                    <td className="text-right whitespace-nowrap">
                      <MenuImprimirOrdenServicio orden={o} />
                      <button
                        onClick={() => setRecepcionOS(o)}
                        title="Recepción digital (fotos + firma)"
                      ...
                      {o.factura ? (
                        <Link to="/ventas" title="Factura creada: valídala y emítela desde Ventas" className="inline-block align-middle mr-3">
                          <Badge tono="green">{o.numeroFactura ?? "borrador"}</Badge>
                        </Link>
                      ) : (
      ...
      {recepcion && (
        <RecepcionRapidaServicio
          onCerrar={() => setRecepcion(false)}
          onCreada={(datos) => {
            setRecepcion(false);
            cargar();
            // Al terminar el alta se abre la recepción digital de la nueva OS
            // para fotos del estado, firma del cliente e impresión de la hoja.
            if (datos?.orden) {
              setRecepcionNueva(true);
              setRecepcionOS(datos.orden);
            }
          }}
        />
      )}
      ...
      {recepcionOS !== undefined && (
        <ModalRecepcionServicio
          orden={recepcionOS}
          recienCreada={recepcionNueva}
          onCerrar={() => {
            setRecepcionOS(undefined);
