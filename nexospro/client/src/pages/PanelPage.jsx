    ...
    <Link to={enlace} className="block">
      ...
      >
        ...
        <div className="relative flex items-start justify-between gap-3">
          <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-slate-500">{etiqueta}</p>
          <span className={`flex items-center justify-center w-9 h-9 rounded-xl border ${t.chip}`}>
            <Icono />
          </span>
        </div>
        <p className="relative text-[1.875rem] leading-tight font-extrabold tracking-tight mt-4 tabular-nums text-white">
          {valor}
        </p>
    ...
    <Link to={enlace} className="block">
      <div className="panel relative overflow-hidden p-4 h-full transition-all duration-150 hover:-translate-y-0.5 hover:border-white/[0.12]">
        <div className="relative flex items-center justify-between gap-2">
          <p className="text-[0.625rem] font-semibold uppercase tracking-[0.14em] text-slate-500">{etiqueta}</p>
    ...
    <>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
        <div className="panel p-6 xl:col-span-2">
          <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-slate-500 mb-5">
            Facturación mensual
          </p>
          <div className="flex items-end gap-3 h-44">
            {(r?.mensual ?? []).map((m) => (
              <div key={m.clave} className="flex-1 flex flex-col items-center gap-2 min-w-0" title={euros(m.total)}>
                ...
                />
                <span className="text-[0.6875rem] text-slate-500 capitalize">{m.etiqueta}</span>
              </div>
            ))}
        ...
        <div className="panel p-6">
          <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.14em] text-slate-500 mb-4">
