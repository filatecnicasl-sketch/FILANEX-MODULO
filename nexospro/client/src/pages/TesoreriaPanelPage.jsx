...
function Tarjeta({ titulo, valor, detalle, tono, to }) {
  ...
  return (
    <Link to={to} className="panel px-6 py-5 block hover:border-accent/40 transition-colors">
      <p className="text-[0.65625rem] font-semibold tracking-[0.12em] uppercase text-slate-400">{titulo}</p>
      <p className={`num text-[1.625rem] font-semibold mt-1.5 ${tonos[tono]}`}>{valor}</p>
  ...
  return (
    <div className="panel">
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
        <h2 className="text-[0.875rem] font-semibold text-[#0f172a]">{titulo}</h2>
    ...
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
        ...
        <div className="panel px-6 py-5">
          ...
          <p className={`num text-[1.625rem] font-semibold mt-1.5 ${pendCobro - pendPago >= 0 ? "text-sky-600" : "text-rose-600"}`}>
