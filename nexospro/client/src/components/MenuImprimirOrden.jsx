...
async function cargarPlantillaEntrada() {
  try {
    const r = await fetch("/api/formatos/default/entrada-taller");
    if (!r.ok) return null;
    return await r.json();
  } catch {
    return null;
...
export async function imprimirHojaEntrada(o) {
  ...
  const formData = {
    ...
    "vehiculo.fecha": fechaEs(o.fechaEntrada),
    "vehiculo.matricula": o.matricula,
    "vehiculo.marca": o.vehiculo?.marca,
    "vehiculo.modelo": o.vehiculo?.modelo,
  ...
  const signatures = {};
  if (o.recepcionDigital?.firma?.imagen) {
    signatures.cliente = o.recepcionDigital.firma.imagen;
  }
