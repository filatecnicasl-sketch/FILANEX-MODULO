import { ELEMENT_NAMES, round1 } from "./editorUtils.js";
import { BUILTIN_IMAGES, BUILTIN_IMAGE_NAMES } from "./assets.js";
import {
  IconTrash, IconCopy, IconLock, IconUnlock, IconChevronUp, IconChevronDown, IconPlus, IconX,
} from "./icons.jsx";

const inputCls =
  "w-full rounded border border-white/10 bg-navy-950 px-1.5 py-1 text-xs text-white outline-none focus:border-accent/60";
const labelCls = "mb-0.5 block text-[0.6875rem] font-medium text-slate-400";
const miniBtn = "rounded p-1 text-slate-400 hover:bg-white/10 hover:text-white";
    ...
    <div>
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (!Number.isNaN(v)) onChange(round1(v));
        }}
      />
    </div>
  );
...
export function PropertiesPanel({
  template, element, onUpdateElement, onUpdateTemplate,
  onRemoveElement, onDuplicateElement, onReorderElement,
}) {
  return (
    <div className="flex w-64 shrink-0 flex-col gap-3 overflow-auto border-l border-white/10 bg-navy-900 p-3">
      {element ? (
        <ElementProps
          element={element}
...
// ---------- Propiedades de página (sin selección) ----------
function PageProps({ template, onUpdateTemplate }) {
  return (
    <>
      ...
      <div className="rounded-md bg-white/5 p-2 text-[0.6875rem] leading-snug text-slate-400">
        Selecciona un elemento de la página para editar sus propiedades. Atajos: <b>Supr</b> elimina,{" "}
        <b>Ctrl+D</b> duplica, <b>flechas</b> mueven.
      </div>
    </>
  );
}
    ...
    <>
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button title="Subir (encima)" className={miniBtn} onClick={() => onReorder(1)}>
            <IconChevronUp />
          </button>
          <button title="Bajar (debajo)" className={miniBtn} onClick={() => onReorder(-1)}>
            <IconChevronDown />
          </button>
          <button
          ...
          >
            {el.locked ? <IconLock className="h-4 w-4 text-amber-400" /> : <IconUnlock />}
          </button>
          <button title="Duplicar (Ctrl+D)" className={miniBtn} onClick={onDuplicate}>
            <IconCopy />
          </button>
          <button title="Eliminar (Supr)" className="rounded p-1 hover:bg-rose-500/10" onClick={onRemove}>
      ...
      {(el.type === "field" || el.type === "textarea") && (
        <>
          ...
          <Txt label="Clave de dato (NEXOSPRO)" value={el.fieldKey} onChange={(v) => onUpdate({ fieldKey: v })} />
      ...
      {el.type === "image" && (
        <>
          <div>
            <span className={labelCls}>Imagen integrada</span>
            <select
              className={inputCls}
              value={BUILTIN_IMAGES[el.src] ? el.src : ""}
              onChange={(e) => e.target.value && onUpdate({ src: e.target.value })}
            >
              <option value="">(personalizada)</option>
              {Object.keys(BUILTIN_IMAGES).map((k) => (
                <option key={k} value={k}>{BUILTIN_IMAGE_NAMES[k]}</option>
              ))}
            </select>
          </div>
          <div>
            <span className={labelCls}>Subir imagen</span>
            <input type="file" accept="image/*" className="text-xs text-slate-400" onChange={upImage} />
          </div>
        </>
      )}
      ...
      {el.type === "rect" && (
        <>
          ...
          <div>
            <span className={labelCls}>Color borde</span>
            <input
              type="color"
              className="h-7 w-full cursor-pointer bg-transparent"
    ...
    <>
      ...
      <div>
        <div className="mb-1 flex items-center justify-between">
          ...
          <button
            className="flex items-center gap-1 rounded bg-white/10 px-1.5 py-0.5 text-[0.6875rem] text-slate-200 hover:bg-white/20"
            onClick={() => onUpdate({ columns: [...el.columns, { title: "NUEVA", width: 0.2 }] })}
          >
            <IconPlus className="h-3 w-3" /> Añadir
          </button>
        </div>
        {el.columns.map((c, i) => (
          <div key={i} className="mb-1 flex items-center gap-1">
            ...
            <input
              ...
              step={0.05}
              min={0.05}
              title="Fracción del ancho"
              onChange={(e) => {
                const v = parseFloat(e.target.value);
                if (!Number.isNaN(v)) setCol(i, { width: Math.max(0.05, v) });
              }}
            />
            <button
              className="rounded p-1 hover:bg-rose-500/10"
              disabled={el.columns.length <= 1}
              onClick={() => onUpdate({ columns: el.columns.filter((_, idx) => idx !== i) })}
            >
              <IconX className="h-3.5 w-3.5 text-rose-400" />
            </button>
          </div>
        ))}
        <div className="text-[0.625rem] text-slate-500">El número es la fracción del ancho total de cada columna.</div>
      </div>
    </>
  );
}
