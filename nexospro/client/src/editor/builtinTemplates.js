...
export function buildContratoCortesia() {
  ...
  return {
    ...
    elements: [
      // Cabecera
      img("{{empresa.logo}}", x, 6, 55, 14),
      txc("{{empresa.nombre}}", x + 70, 8, 110, 6, 11, true, "right"),
      txc("{{empresa.direccion}}", x + 70, 14, 110, 4, 8, false, "right", "#333"),
      txc("{{empresa.telefono}}", x + 70, 18, 110, 4, 8, false, "right", "#333"),

      txc("VEHÍCULOS DE SUSTITUCIÓN", x, 22, w, 7, fsTitle, true, "center"),
      txc("Contrato de prestación con participación forfait Nº", x, 29, 145, 5, 9, false, "center"),
      fl("", "prestamo.numero", 160, 29, 38, 6),

      // USUARIO
      ...seccion("USUARIO", x, yUsuario, w),
      txc("Atención: Toda persona que conduzca el vehículo prestado, debe ser mayor de edad y poseer un permiso o licencia de conducir desde hace más de 1 año, y no estar en suspenso o anulado.", x + 1, yUsuario + 6, w - 2, 5.5, 6.5, false, "justify"),
      fl("Apellidos", "cliente.apellidos", x, yUsuario + 12, medio - 2),
      fl("Nombre", "cliente.nombre", x + medio, yUsuario + 12, medio - 2),
      fl("Fecha de nacimiento", "cliente.fechaNacimiento", x, yUsuario + 19, medio - 2),
      fl("Lugar de nacimiento", "cliente.lugarNacimiento", x + medio, yUsuario + 19, medio - 2),
      fl("Dirección", "cliente.direccion", x, yUsuario + 26, w),
      fl("Tlf. Particular", "cliente.telefono", x, yUsuario + 33, medio - 2),
      fl("Móvil", "cliente.movil", x + medio, yUsuario + 33, medio - 2),
      fl("Trabajo", "cliente.telefonoTrabajo", x, yUsuario + 40, w),
      fl("Permiso de conducir nº", "cliente.permisoNumero", x, yUsuario + 47, w),
      fl("Expedido el", "cliente.permisoExpedicion", x, yUsuario + 54, medio - 2),
      fl("en", "cliente.permisoLugar", x + medio, yUsuario + 54, medio - 2),
      fl("Apellidos", "cliente2.apellidos", x, yUsuario + 61, medio - 2),
      fl("Nombre", "cliente2.nombre", x + medio, yUsuario + 61, medio - 2),
      fl("Permiso de conducir nº", "cliente2.permisoNumero", x, yUsuario + 68, w),
      fl("Expedido el", "cliente2.permisoExpedicion", x, yUsuario + 75, medio - 2),
      fl("en", "cliente2.permisoLugar", x + medio, yUsuario + 75, medio - 2),
      fl("Eventualmente, otros conductores admitidos", "cliente.otrosConductores", x, yUsuario + 82, w),

      // VEHÍCULO PUESTO A DISPOSICIÓN
      ...seccion("VEHÍCULO PUESTO A DISPOSICIÓN", x, yVehDisp, w),
      txc("Vehículo puesto a disposición Nº", x + 1, yVehDisp + 7, 56, 5, 7, true),
      fl("Marca", "vehiculo.marca", x + 58, yVehDisp + 6, 38, 6),
      fl("Modelo", "vehiculo.modelo", x + 98, yVehDisp + 6, 40, 6),
      fl("Nº Matrícula", "vehiculo.matricula", x + 140, yVehDisp + 6, 46, 6),
      fl("V.I.N.", "vehiculo.vin", x, yVehDisp + 14, medio - 2),
      fl("Participación forfait diaria", "vehiculo.forfaitDiario", x + medio, yVehDisp + 14, 52, 6),
      txc("PTE €", x + medio + 54, yVehDisp + 15, 18, 5, 7, true),
      ...
      txc("Fecha y hora:", x + 1, yKm + 8, 35, 5.5, 7, true),
      txc("Km. En el contador:", x + 1, yKm + 15, 35, 5.5, 7, true),
      txc("Nivel de carburante:", x + 1, yKm + 22, 35, 5.5, 7, true),

      // ESTADO DEL VEHÍCULO
      ...seccion("ESTADO DEL VEHÍCULO", x, yEstado, medio - 2),
      txc("[imagen lateral]", x + 5, yEstado + 7, 80, 5, 6, false, "center", "#999"),
      txc("[imagen superior]", x + 5, yEstado + 24, 80, 5, 6, false, "center", "#999"),
      txc("[imagen frontal]", x + 5, yEstado + 41, 38, 5, 6, false, "center", "#999"),
      txc("[imagen trasera]", x + 47, yEstado + 41, 38, 5, 6, false, "center", "#999"),

      // Observaciones
      box2(x + medio, yEstado, medio - 2, 32, 0.8),
      txc("OBSERVACIONES", x + medio + 1, yEstado + 1, medio - 4, 5, fsSec, true, "center"),
      { id: id(), type: "textarea", x: x + medio + 2, y: yEstado + 7, w: medio - 6, h: 24, label: "", fieldKey: "vehiculo.observaciones", fontSize: 7.5, boxed: false },

      // SEGURO
      ...seccion("SEGURO", x, ySeguro, w),
      fl("Asegurador", "seguro.asegurador", x, ySeguro + 6, medio - 2),
      fl("Nº de Contrato", "seguro.numeroContrato", x + medio, ySeguro + 6, medio - 2),
      txc("Montante de las franquicias:", x + 1, ySeguro + 12, w - 2, 5, 7, true),
      fl("Daños causados a terceros", "seguro.franquiciaTerceros", x, ySeguro + 18, tercio - 2),
      fl("Daños causados al vehículo", "seguro.franquiciaVehiculo", x + tercio, ySeguro + 18, tercio - 2),
      fl("Robo/Vandalismo", "seguro.franquiciaRobo", x + tercio * 2, ySeguro + 18, tercio - 2),
      chk("Rescate de franquicias: SI", "seguro.rescateSi", x + 1, ySeguro + 25, 34, 5, 7),
      chk("NO", "seguro.rescateNo", x + 38, ySeguro + 25, 18, 5, 7),
      chk("Transferencia de seguro: SI", "seguro.transferenciaSi", x + 80, ySeguro + 25, 44, 5, 7),
      chk("NO", "seguro.transferenciaNo", x + 128, ySeguro + 25, 18, 5, 7),
      fl("Montante del rescate por día", "seguro.rescatePorDia", x, ySeguro + 31, w),
      txc("*Aporta un justificante de alteración de seguro", x + 1, ySeguro + 37, w - 2, 4, 6, false, "left", "#555"),
      ...
      fl("V.I.N.", "reparacion.vin", x, yRep + 14, w),
      fl("Nº de O.R.", "reparacion.numeroOrden", x, yRep + 21, medio - 2),
      fl("Entrega prevista", "reparacion.entregaPrevista", x + medio, yRep + 21, medio - 2),

      // Firmas
      box2(x, yFirmas, tercio - 2, 24, 0.8),
      txc("Declaro haber tenido conocimiento de las condiciones generales de protección indicadas al dorso del presente Contrato.", x + 2, yFirmas + 2, tercio - 6, 12, 6, true, "center"),
      txc("Realizado en _______ a ____/____/________", x + 2, yFirmas + 16, tercio - 6, 4.5, 6, false, "center"),
      txc("Firma del cliente", x + 2, yFirmas + 21, tercio - 6, 4.5, 6, false, "center"),

      box2(x + tercio, yFirmas, tercio - 2, 24, 0.8),
      txc("El cliente (firma)", x + tercio + 2, yFirmas + 2, tercio - 6, 4.5, 6, true, "center"),
      txc("El concesionario (nombre y firma)", x + tercio + 2, yFirmas + 7, tercio - 6, 4.5, 6, true, "center"),
      txc("Fecha: ____/____/________", x + tercio + 2, yFirmas + 19, tercio - 6, 4.5, 6, false, "center"),

      box2(x + tercio * 2, yFirmas, tercio - 2, 24, 0.8),
      txc("Al retorno, el cliente (firma)", x + tercio * 2 + 2, yFirmas + 2, tercio - 6, 12, 6, true, "center"),
      txc("Fecha: ____/____/________", x + tercio * 2 + 2, yFirmas + 19, tercio - 6, 4.5, 6, false, "center"),

      txc("ATENCIÓN: Este contrato debe ir acompañado al vehículo durante toda la duración de la prestación.", x, yFirmas + 28, w, 4.5, 7, true, "center"),
    ].flat(),
  };
}
